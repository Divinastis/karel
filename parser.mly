...
| 			FRONT_IS_BLOCKED
					{ let b = new_temp() in gen (INVOKE (is_blocked,	1,	b)) ; b }
| 			LEFT_IS_CLEAR
					{ let b = new_temp() in gen (INVOKE (is_clear,	2,	b)) ; b }
| 			LEFT_IS_BLOCKED
					{ let b = new_temp() in gen (INVOKE (is_blocked,	2,	b)) ; b }
| 			RIGHT_IS_CLEAR
					{ let b = new_temp() in gen (INVOKE (is_clear,	3,	b)) ; b }
| 			RIGHT_IS_BLOCKED
					{ let b = new_temp() in gen (INVOKE (is_blocked,	3,	b)) ; b }
| 			NEXT_TO_A_BEEPER
					{ let a = new_temp() in gen (INVOKE (next_beeper,	a,	0)) ; a }
| 			NOT_NEXT_TO_A_BEEPER
					{ let a = new_temp() in gen (INVOKE (no_next_beeper,	a,	0)) ; a }
|				FACING_NORTH
					{ let b = new_temp() in gen (INVOKE (facing,	1,	b)) ; b }
| 			NOT_FACING_NORTH
					{ let b = new_temp() in gen (INVOKE (not_facing,	1,	b)) ; b }
| 			FACING_EAST
					{ let b = new_temp() in gen (INVOKE (facing,	2,	b)) ; b }
| 			NOT_FACING_EAST
					{ let b = new_temp() in gen (INVOKE (not_facing,	2,	b)) ; b }
| 			FACING_SOUTH
					{ let b = new_temp() in gen (INVOKE (facing,	3,	b)) ; b }
|				NOT_FACING_SOUTH
					{ let b = new_temp() in gen (INVOKE (not_facing,	3,	b)) ; b }
| 			FACING_WEST
					{ let b = new_temp() in gen (INVOKE (facing,	4,	b)) ; b }
| 			NOT_FACING_WEST
					{ let b = new_temp() in gen (INVOKE (not_facing,	4,	b)) ; b }
|				ANY_BEEPERS_IN_BEEPER_BAG
					{ let a = new_temp() in gen (INVOKE (any_beeper,	a,	0)) ; a }
| 			NO_BEEPERS_IN_BEEPER_BAG
					{ let a = new_temp() in gen (INVOKE (no_beeper,	a,	0)) ; a }
;
